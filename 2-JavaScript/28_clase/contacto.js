...
function renderContactDetail(contacto) {
    const contactoDetail = document.getElementById('contacto-detail');

    contactoDetail.innerHTML = `
        <h2>${contacto.name}</h2>
        <p>Unseen messages: ${contacto.unseen_messages}</p>
        <p>Last message: ${contacto.last_message}</p>
        <p>Last message date: ${contacto.last_message_date}</p>
        <img src="${contacto.image}" alt="${contacto.name}">
    `;
}

const contacto = obtenerContactoPorId(contactoId);
renderContactDetail(contacto);
